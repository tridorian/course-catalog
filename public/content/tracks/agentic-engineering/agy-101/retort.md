...
*   **Critical Version Constraint (v1.23.2 or higher):**
    *   **Pedagogical Rationale:** The course establishes a baseline version of Antigravity `v1.23.2` or higher. The curriculum is kept up-to-date with `agy-easy-install`, verifying compatibility across all newer versions, including `v2.0.0+`. This ensures learners can utilize current tooling versions without breaking local verification targets or policies.
