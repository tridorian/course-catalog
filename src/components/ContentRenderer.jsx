    ...
    <div className="space-y-6">
      {blocks.map((block, index) => {
        switch (block.type) {
          ...
          case 'h2':
            return <h2 key={index} className="text-2xl font-bold text-white mt-8 mb-4 border-b border-border-main pb-2">{block.content}</h2>;

          case 'h3':
            return <h3 key={index} className="text-xl font-bold text-accent-text mt-8 mb-2">{block.content}</h3>;

          case 'p':
            return (
              <p key={index} className={`text-lg text-text-muted ${block.italic ? 'italic' : ''}`}>
          ...
          case 'numbered_list':
            return (
              <ol key={index} className="list-decimal pl-5 space-y-6 text-text-muted mt-6">
                {block.items.map((item, i) => (
                  <li key={i}>
                    {typeof item === 'string' ? (
                      renderMarkdown(item)
                      ...
                      <>
                        ...
                        {item.prompt && (
                           <div className="bg-black/50 p-4 rounded-lg text-text-muted font-mono text-sm border border-border-main mt-2 mb-2 italic">
                            Prompt: "{item.prompt}"
                          </div>
                        )}
          ...
          case 'tier_card':
            return (
              <div key={index} className={`p-4 border rounded-lg relative overflow-hidden ${block.recommended ? 'border-accent bg-muted' : 'border-border-main bg-panel'}`}>
                {block.recommended && (
                ...
                <p className="text-sm text-text-muted mb-3">{block.description}</p>
          ...
          case 'feature_card':
            ...
            return (
              <div key={index} className="bg-panel p-6 rounded-xl border border-border-main">
                <h3 className="text-xl font-bold text-accent-text flex items-center gap-2 mb-3">
                  <FeatureIcon size={24}/> {block.title}
                </h3>
                <p className="text-text-muted mb-4">{renderMarkdown(block.content)}</p>
                {block.prompt && (
                  <div className="bg-black/50 p-3 rounded text-text-muted font-mono text-sm border-l-2 border-accent">
                    Prompt: "{block.prompt}"
                  </div>
                )}
              ...
              <div key={index} className="space-y-6 mt-8">
                {block.items.map((item, i) => (
                  <div key={i} className={`border-l-2 border-border-main pl-6 pb-6 relative ${i === block.items.length - 1 ? 'border-transparent' : ''}`}>
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">{item.title}</h3>
                    <p className="text-text-muted mb-2">{renderMarkdown(item.content)}</p>
                    {item.code && <CodeBlock code={item.code} />}
                    {item.prompt && (
                      <div className="bg-black/50 p-3 rounded text-text-muted font-mono text-sm italic border border-border-main">
                        "{item.prompt}"
                      </div>
                    )}
                  </div>
          ...
          case 'recovery_options':
             return (
               <ul key={index} className="space-y-4 text-text-muted">
                 {block.items.map((item, i) => (
                   <li key={i} className={`p-4 rounded border ${item.highlight ? 'border-accent bg-muted relative overflow-hidden' : 'border-border-main bg-muted'}`}>
                     {item.highlight && (
          ...
          case 'next_steps':
            return (
              <div key={index} className="max-w-2xl mx-auto mt-12 bg-muted p-8 rounded-2xl border border-border-main text-left">
